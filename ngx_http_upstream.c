...
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
    ...
    //原始请求中的cleanup链表添加新成员
    cln = ngx_http_cleanup_add(r, 0);
    if (cln == NULL) {
        ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
    //回调方法。说明当前请求结束一定会调用ngx_http_upstream_cleanup方法
...
ngx_http_upstream_handler(ngx_event_t *ev)
{
    ...
    //取得表示upstream机制的ngx_http_upstream_t结构体
    u = r->upstream;
    //取得与客户端的连接
    c = r->connection;
...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    //向上游服务器发送请求的方法是ngx_http_upstream_send_request_handler
    u->write_event_handler = ngx_http_upstream_send_request_handler;//检查请求状态
    ...
    ngx_uint_t do_write)
{
    ...
    //防止事件再次触发而又调用ngx_http_upstream_send_request
    ...
    ngx_http_upstream_t *u, ngx_uint_t do_write)
{
    ...
    if (!r->request_body_no_buffering) {
        ...
        //ngx_output_chain方法向上游服务器发送ngx_http_upstream_t结构体中的request_bufs链表
...
{
    ngx_connection_t  *c;
    //获取与上游服务器间表示连接的ngx_connection_t
    c = u->peer.connection;
    ...
    //向上游服务器发送的请求已经超时
    if (c->write->timedout) {
        //ngx_http_upstream_next决定重连或者结束
        ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_TIMEOUT);
        return;
    }
...
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    //ngx_http_upstream_process_headers处理已经解析出的头部,会将头部设置到ngx_http_request_t结构体中的Headers_out成员中
    if (ngx_http_upstream_process_headers(r, u) != NGX_OK) {
        return;
    }
    //需要转发响应头
    ...
    if (u->input_filter == NULL) {
        ...
        u->input_filter_ctx = r;//用于传递http模块自定义的数据结构
    }
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
    //buffering为0说明以下流网速优先，使用固定大小的缓存或临时文件
    if (!u->buffering) {
        ...
        if (n) {
            ...
            //重置buffer
            u->buffer.pos = u->buffer.start;//pos一般指向未经处理的响应
            u->buffer.last = u->buffer.start;//last一般指向刚刚接收到的响应
            //NGX_HTTP_FLUSH催促
    ...
    ngx_uint_t do_write)
{
    ...
    for ( ;; ) {
        //do_write为1表示向下游发送响应
        if (do_write) {
            //out_bufs表示上一次向下游转发响应到现在这段时间内接收来自上游的缓存响应
            //busy_bufs表示上一次向下游转发响应时还没有发送完的内容
            if (u->out_bufs || u->busy_bufs) {
                ...
                //更新，清空out_bufs，如果没有一次性将out_bufs全部发送完则添加到Busf_bufs中
        ...
        size = b->end - b->last;
        //upstream->read->ready表示与上游的连接上确定有可读事件
...
//试图在buffer缓冲区存放全部的响应主体
static ngx_int_t
ngx_http_upstream_non_buffered_filter(void *data, ssize_t bytes)
{
    //data指向ngx_http_upstream_t中的input_filter_ctx
    ...
    u = r->upstream;
    //ll指向最后一个缓冲区的地址，cl指向链表中最后一个ngx_chain_t元素的next成品
    ...
    *ll = cl;
    ...
    cl->buf->pos = b->last;
    b->last += bytes;//buffer需要保存这次收到的包体数据
    cl->buf->last = b->last;
    ...
    ngx_http_upstream_t *u)
{
    ...
    if (rev->timedout) {
        ...
        //ngx_event_pipe中的第二个参数为0表示仅需要由上游客户端接收响应
