...
ngx_http_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    http_ctx = cf->ctx;
    //server的ctx指向所属的http块下的ngx_http_conf_ctx_t结构体的main_conf
    ctx->main_conf = http_ctx->main_conf;

    //srv_conf loc_conf重新分配指针数组
    ...
    for (i = 0; cf->cycle->modules[i]; i++) {
      ...
      //将ngx_http_core_src_conf_t(对应着正常解析的server)添加到全局的ngx_http_core_main_conf_t结构体的servers数组中
...
ngx_http_core_location(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    //ctx的main_conf和srv_conf都指向所属的server级别的ngx_http_conf_ctx_t结构体的main_conf和srv_conf指针数组
    pctx = cf->ctx;
