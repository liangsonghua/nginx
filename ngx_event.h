...
struct ngx_event_s {
    ...
    unsigned         accept:1;//可以建立新的连接

    /* used to detect the stale events in kqueue and epoll */
    unsigned         instance:1;//当前事件是否过期
    ...
    unsigned         eof:1;//字符流已经结束
    unsigned         error:1;

    unsigned         timedout:1;
    unsigned         timer_set:1;//事件存在定时器中

    unsigned         delayed:1;//需要延时处理

    unsigned         deferred_accept:1;//延时建立连接

    /* the pending eof reported by kqueue, epoll or in aio chain operation */
    unsigned         pending_eof:1;//等待字符流结束

    unsigned         posted:1;//在处理Post事件时，1表示当前事件已经准备就绪
    ...
    ngx_event_handler_pt  handler;//事件发生时的处理方法
    ...
    /* the posted queue */
    ngx_queue_t      queue;//post事件会构成一个队列再处理
