    ...
    ngx_http_module_t *module, ngx_uint_t ctx_index)
{
    ...
    //从ngx_http_core_main_conf_t结构体的动态数组中servers可以获取到所有的ngx_http_core_srv_conf_t
    cscfp = cmcf->servers.elts;
    //在http块下的全局ngx_http_conf_ctx_t结构体
    ...
    for (s = 0; s < cmcf->servers.nelts; s++) {
        ...
        if (module->merge_srv_conf) {
            //saved.srv_conf[ctx_index]是当前模块在http块中产生的ngx_http_core_srv_conf_t
             // cscfp[s]->ctx->srv_conf[ctx_index]是server块下创建的结构体
        ...
        if (module->merge_loc_conf) {
            ...
            //cscfp[s]->ctx->loc_conf是server块下相关的所有location相关的结构体合并
            ...
            //clcf是server块下由create_loc_conf创建的结构体
            clcf = cscfp[s]->ctx->loc_conf[ngx_http_core_module.ctx_index];
