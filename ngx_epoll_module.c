...
static int                  ep = -1;
static struct epoll_event  *event_list;//相当于epoll_wait中的参数events
static ngx_uint_t           nevents;//相当于epoll_events指明了epoll_wait一次返回的最大事件数
...
static ngx_command_t  ngx_epoll_commands[] = {
    ...
    { ngx_string("epoll_events"),
      ...
      NULL },
      //指明在开启异步io且使用io_setup系统调用初始化异步io上下文环境时，初始分配的异步io事件个数
...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    for (i = 0; i < events; i++) {
        ...
        rev = c->read;
        //判断是否过期
        //epoll_wait返回多个事件时，ngx_get_connection获取新连接时已经将instance标志位置反
        //因为有可能第三个事件申请的连接刚好是第一个事件释放的连接
        //所有不能直接通过fd==-1判断是否过期
