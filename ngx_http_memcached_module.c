...
//ngx_http_module_t的8个回调函数，主要是负责设置和合并http/server/location区域的配置项
...
//用于解析上游服务器返回的基于TCP的响应头部
//主要是解析r->upstream->buffer缓冲区
static ngx_int_t
...
/*重新实现filter的场景
1.在转发上游响应到下游的同时，需要做一些特殊的处理
2.当无须在上下流转发响应时，并不想等待接收完全部的上流响应后才开始处理请求*/
static ngx_int_t
...
ngx_http_memcached_create_loc_conf(ngx_conf_t *cf)
{
    ...
    //??为什么没有初始化hide_headers/pass_headers
    //ngx_http_memcached_merge_loc_conf中也没有
