...
struct ngx_module_s {
    //默认使用NGX_MODULE_V1
    ngx_uint_t            ctx_index;//当前模块在此类中的序号
    ngx_uint_t            index;
    ...
    ngx_int_t           (*init_master)(ngx_log_t *log);//master进程启动时将调用init_master一般为NULL

    ngx_int_t           (*init_module)(ngx_cycle_t *cycle);//初始化所有模块时被调用

    ngx_int_t           (*init_process)(ngx_cycle_t *cycle);//正常服务前被调用
    ngx_int_t           (*init_thread)(ngx_cycle_t *cycle);//NGINX不支持多线程
    void                (*exit_thread)(ngx_cycle_t *cycle);
    void                (*exit_process)(ngx_cycle_t *cycle);

    void                (*exit_master)(ngx_cycle_t *cycle);
    //保留字段，默认使用NGX_MODULE_V1_PADDING
