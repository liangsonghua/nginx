...
// 检查preread_bufs预读缓冲区链表指针是否为空
// preread_bufs为空，检查free_raw_bufs链表指针是否为空
// free_raw_bufs为空，检查allocated是否小于bufs.num
// allocate大于或等于bufs.num,检查是否可向下游发送响应
// 下游写事件不可用，检查临时文件是否达到max_temp_file_size
static ngx_int_t
    ...
    for ( ;; ) {
        ...
        if (p->preread_bufs) {
            ...
            if (p->free_raw_bufs) {
                ...
                chain->buf = b;
                chain->next = NULL;
            //检查ready和delayed标志，判断是否应当由向下游发送响应以清理缓冲区
                //delayed为0表示并不是由于限速才使得写事件准备好
            } else if (!p->cacheable
                       && p->downstream->data == p->output_ctx
...
//注意：因为out链表中的缓存区内容在响应中的位置要比In链表更靠前，所以要优先发送给下游
static ngx_int_t
ngx_event_pipe_write_to_downstream(ngx_event_pipe_t *p)
{
    ...
    for ( ;; ) {
        ...
        out = NULL;
        //如果bsize大于配置当中的busy_size，则不再发送Out和In缓存区，优先处理ngx_http_request_t结构体中的Out
